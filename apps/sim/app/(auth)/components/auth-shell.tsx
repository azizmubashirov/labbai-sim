 ...
 *
 * It pins the `light` token layer (so the platform's light-mode `var(--*)` tokens
 * resolve regardless of the visitor's theme), uses the canvas/`--text-primary`
 * surface, and renders a logo-only header. The single content column is centered
 * and capped for a calm single-form layout.
 */
    ...
    <div className='light relative flex min-h-screen flex-col bg-[var(--bg)] text-[var(--text-primary)]'>
      <header>
        <nav className='mx-auto flex w-full max-w-[1446px] items-center px-12 py-4 max-sm:px-5 max-lg:px-8'>
          <a href='/' aria-label='Home' className='flex h-[30px] items-center'>
            <SimWordmark />
          </a>
        </nav>
      </header>
