...
const { mockUseUserPermissionConfig, mockUsePermissionGroups } = vi.hoisted(() => ({
...
describe('AccessControl entitlement states', () => {
  ...
  it('shows an entitlement failure instead of an admin notice when access is unknown', () => {
    mockUseUserPermissionConfig.mockReturnValue({
      data: undefined,
      error: new Error('Access Control access failed'),
      isPending: false,
    })
    ...
    expect(container.textContent).toContain('Access Control access failed')
    expect(container.textContent).not.toContain('Only organization admins can manage')
  })
})
