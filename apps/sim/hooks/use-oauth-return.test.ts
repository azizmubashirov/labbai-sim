...
describe('buildKnowledgeBaseOAuthReturnUrl', () => {
  ...
  it('returns to the knowledge base when no connector form is pending', () => {
    expect(buildKnowledgeBaseOAuthReturnUrl('workspace-1', 'kb-1')).toBe(
      '/workspace/workspace-1/knowledge/kb-1'
    )
  })

  it('sends organization-owned sources to the app entry', () => {
    expect(
      buildKnowledgeBaseOAuthReturnUrl(
