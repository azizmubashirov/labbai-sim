...
const { queryClient, clearChat } = vi.hoisted(() => ({
  clearChat: vi.fn(),
  queryClient: {
...
describe('tasks query boundary parsing', () => {
  ...
  it('clears chat state only from the successful single-delete callback', async () => {
  ...
  it('clears each successful bulk delete even when a sibling delete fails', async () => {
    const mutation = useDeleteMothershipChats('workspace-1') as unknown as {
      mutationFn: (chatIds: string[]) => Promise<void>
    }
    vi.mocked(fetch)
