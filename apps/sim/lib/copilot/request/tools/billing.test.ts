...
describe('handleBillingLimitResponse', () => {
  ...
  it('keeps member-scope limit messaging even when an org payer snapshot is present', async () => {
    const onEvent = vi.fn()
    const execContext = createExecutionContext({
      actorUserId: 'actor-1',
      workspaceId: 'ws-1',
    ...
    await handleBillingLimitResponse(
      ...
      {
        scope: 'member',
        message: 'You have reached your personal member allocation for this organization.',
      }
    )
    ...
    expect(onEvent.mock.calls[0]?.[0]).toMatchObject({
      payload: {
        text: expect.stringMatching(
          /"action":"increase_limit".*personal member allocation for this organization/
        ),
      },
    })
  })
