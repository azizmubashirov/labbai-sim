 ...
 * of that account still carrying the error is due now, with its failure count cleared.
 * Connectors paused or disabled for another reason keep their state, and a connector that
...
export async function resumeConnectorsAfterCredentialReconnect(
  ...
  const repairedAccounts = eq(credential.accountId, accountId)
  await db
