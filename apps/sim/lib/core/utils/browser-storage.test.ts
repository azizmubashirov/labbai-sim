...
describe('MothershipHandoffStorage', () => {
  ...
  it('discards a legacy organization handoff instead of replaying it in a workspace', () => {
    localStorage.setItem(
      STORAGE_KEYS.MOTHERSHIP_HANDOFF,
      JSON.stringify({ message: 'Find the policy', organizationId: 'org-1', timestamp: Date.now() })
    )
